...
public:
	...
	void Recognition(unsigned char* pInput, double* pOutput, int oCount);
	...
	//c++°æ±¾
	void ReadNN_Cpp(ifstream& NN_file);
	void WriteNN_Cpp(ofstream& NN_file);
	void LoadNN_Cpp(TCHAR* NN_name);

	//c°æ±¾
	void LoadNN_C(char* NN_name);
	void ReadNN_C(FILE* NN_file);
	void WriteNN_C(FILE* NN_file);
...
//	void PeriodicWeightSanityCheck();  // check if weights are "reasonable"
	...
	NNLayer();
	NNLayer( tstring str, NNLayer* pPrev = NULL );
	virtual ~NNLayer();
...
public:
	NNWeight();
	NNWeight( tstring str, double val = 0.0 );
	virtual ~NNWeight();
