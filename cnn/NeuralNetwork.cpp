...
void NeuralNetwork::Recognition(unsigned char* pInput, double* pOutput, int oCount)
{
	int ii, jj;
	...
	Calculate(inputVector, 841, pOutput, oCount, NULL);
}
...
//c++版本读写分类器
void NeuralNetwork::ReadNN_Cpp(ifstream& NN_file)
{
	double eta; 
	NN_file >> eta;
	m_etaLearningRate = eta;  // two-step storage is needed since m_etaLearningRate is "volatile"
	...
	for ( int ii=0; ii<nLayers; ++ii )
	{
		...
		pLayer->ReadNN_Cpp( NN_file );
	}

}

void NeuralNetwork::WriteNN_Cpp(ofstream& NN_file)
{
	// TODO: add storing code here

	NN_file << m_etaLearningRate <<endl;

	NN_file << m_Layers.size() <<endl;

	VectorLayers::iterator lit;

	for( lit=m_Layers.begin(); lit<m_Layers.end(); lit++ )
	{
		(*lit)->WriteNN_Cpp(NN_file);
	}
}

void NeuralNetwork::LoadNN_Cpp(TCHAR* NN_name)
{
	ifstream fileCNN(NN_name);
	//fileCNN.open;
	if(!fileCNN)
	{
	...
	Initialize();//删除原有的神经网络模型
	ReadNN_Cpp(fileCNN);
	fileCNN.close();
}

//c版本读写分类器
void NeuralNetwork::ReadNN_C(FILE* NN_file)
{
	double eta;
	fscanf_s(NN_file, "%lf", &eta);
	m_etaLearningRate = eta;  // two-step storage is needed since m_etaLearningRate is "volatile"

	int nLayers;
	NNLayer* pLayer = NULL;
	fscanf(NN_file, "%d", &nLayers);
	...
	for ( int ii=0; ii<nLayers; ++ii )
	{
		...
		pLayer->ReadNN_C( NN_file );
	}
}
void NeuralNetwork::WriteNN_C(FILE* NN_file)
{
	fprintf(NN_file, "%lf ", m_etaLearningRate);
	fprintf(NN_file, "%d ", m_Layers.size());
	VectorLayers::iterator lit;

	for( lit=m_Layers.begin(); lit<m_Layers.end(); lit++ )
	{
		(*lit)->WriteNN_C(NN_file);
	}
}

void NeuralNetwork::LoadNN_C(char* NN_name)
{
	FILE* fileCNN = NULL;
	fileCNN = fopen(NN_name, "r");
	if(NULL == fileCNN)
	{	
		printf("cann't open classfier file(%s)", NN_name);
		return;
...
NNLayer::NNLayer( tstring str, NNLayer* pPrev /* =NULL */ ) :
...
void NNLayer::ReadNN_Cpp(ifstream& NN_file)
{
	int ii,jj;
	//string str;
	//NN_file >> str;
	...
	{
		//NN_file >> str;
		pNeuron = new NNNeuron(  _T(" ")/*str.c_str()*/ );
		m_Neurons.push_back( pNeuron );

		NN_file >> iNumConnections;

		for ( jj=0; jj<iNumConnections; ++jj )
		{
			NN_file >> conn.NeuronIndex;
			NN_file >> conn.WeightIndex;
	...
	for ( jj=0; jj<iNumWeights; ++jj )
	{
		...
		pWeight = new NNWeight(  _T(" ")/*str.c_str()*/, value );
		m_Weights.push_back( pWeight );
	}
}

void NNLayer::WriteNN_Cpp(ofstream& NN_file)
{
	...
	//NN_file << label.c_str() <<"\t";
	//NN_file << "a" <<"\t";
	NN_file << m_Neurons.size() <<" ";
	NN_file << m_Weights.size() <<" ";


	for ( nit=m_Neurons.begin(); nit<m_Neurons.end(); nit++ )
	{
		NNNeuron& n = *(*nit);
		//NN_file << n.label.c_str() <<"\t";
		//NN_file << "a" <<"\t";
		NN_file << n.m_Connections.size() <<" ";

		for ( cit=n.m_Connections.begin(); cit<n.m_Connections.end(); cit++ )
		{
			NN_file << (*cit).NeuronIndex <<" ";
			NN_file << (*cit).WeightIndex <<" ";
		}
	}

	for ( wit=m_Weights.begin(); wit<m_Weights.end(); wit++ )
	{
		//NN_file << (*wit)->label.c_str() <<"\t";
		//NN_file << "a" <<"\t";
		NN_file << (*wit)->value <<" ";
	}
}

void NNLayer::ReadNN_C(FILE* NN_file)
{
	int ii,jj;
	int iNumNeurons, iNumWeights, iNumConnections;
	UINT temp;
	...
	fscanf(NN_file, "%d", &iNumNeurons);
	fscanf(NN_file, "%d", &iNumWeights);
	...
	for ( ii=0; ii<iNumNeurons; ++ii )
	{
		...
		fscanf(NN_file, "%d", &iNumConnections);

		for ( jj=0; jj<iNumConnections; ++jj )
		{
			fscanf(NN_file, "%u", &temp);
			conn.NeuronIndex = temp;
			fscanf(NN_file, "%u", &temp);
			conn.WeightIndex = temp;
...
void NNLayer::WriteNN_C(FILE* NN_file)
{
	VectorNeurons::iterator nit;
	VectorWeights::iterator wit;
	VectorConnections::iterator cit;

	fprintf(NN_file, "%d ", m_Neurons.size());
	fprintf(NN_file, "%d ", m_Weights.size());

	for ( nit=m_Neurons.begin(); nit<m_Neurons.end(); nit++ )
	{
		NNNeuron& n = *(*nit);
		
		fprintf(NN_file, "%d ", n.m_Connections.size());

		for ( cit=n.m_Connections.begin(); cit<n.m_Connections.end(); cit++ )
		{
			fprintf(NN_file, "%u ", (*cit).NeuronIndex);
			fprintf(NN_file, "%u ", (*cit).WeightIndex);
		}
	}

	for ( wit=m_Weights.begin(); wit<m_Weights.end(); wit++ )
	{
		fprintf(NN_file, "%lf ", (*wit)->value);
	}
...
NNWeight::NNWeight( tstring str, double val /* =0.0 */ ) :
label( str ),
